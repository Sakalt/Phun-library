...
export class EikyuDate {
    ...
    constructor(inputType: string, valueOrYear: number, season: number = 1, point: number = 1, day: number = 1, hour: number = 0, period: number = 0, minute: number = 0, second: number = 0, milisecond: number = 0) {
        if (inputType == "total") {
    ...
    toMiliseconds(yea: number, sea: number = 1, poi: number = 1, day: number = 1, hou: number = 0, per: number = 0, min: number = 0, sec: number = 0, mil: number = 0) {
        const difYear = yea
    ...
    static toEarth(eikyuDate: number) {
    ...
    toEikyuFormat() {
        ...
        if (numOfPeriod40 >= 10) {numOfPeriod40 = 9} //大閏年の最後の日を前の年へ
    ...
    getPhunBase12() {
        ...
        return `${obj.yea}年${obj.sea}季${obj.poi}旬${obj.day}日　${obj.hou}時${obj.per}刻${obj.min}分${obj.sec}秒`
    }
    getPhunDay() {
        const funNumber = new FunNumber()
        const eikyuObj = this.toEikyuFormat()
